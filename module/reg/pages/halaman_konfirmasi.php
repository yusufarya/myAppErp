...
$query 	= mysql_query("SELECT * FROM invoice WHERE inv_no = '$_GET[inv_no]'") or die (mysql_error());
while ( $row	= mysql_fetch_array($query)) {
	...
	$receiptBank 	= $row['receipt_bank'];
	$paidName 		= $row['paid_name'];
	$ppn 			= $row['ppn'];
	...
	$total_amount 	= $total_amount1 + $ppn + $initial_amount;
}

$custID = $_SESSION['custID'];
$inv_no	= $_GET['inv_no'];

$scDbname = mysql_query("SELECT I.id, I.inv_no, I.cust_id, D.order_id, CO.dbname AS dbname, C.email AS email FROM `invoice` I 
							...
							<div id="form" >
								<form id="form-konfirmasi" class="" method="post" action="act_konfirmasi.php" enctype="multipart/form-data">
									<div class="row">
										<div class="col-lg-6 col-md-6">
											<div class="form-group">
												...
												<input type="text" name="inv_no" id="inv_no" class="form-control " placeholder="Masukkan nomor invoice" value="<?=$_GET['inv_no'];?>" readonly>
											</div>
										</div>
										<div class="col-lg-6 col-md-6">
											<div class="form-group">
									...
									<div class="row">
										<div class="col-lg-4 col-md-4">
											<div class="form-group">
												...
												<input type="text" name="tgl_inv" id="tgl_inv" class="form-control disabled" placeholder="Tanggal invoice" value="<?=date('d-m-Y', strtotime($inv_date));?>" readonly>
											</div>
										</div>
										<div class="col-lg-4 col-md-4">
											<div class="form-group">
												<label class="control-label">Total Amount</label>
												<input type="text" name="total_amount" id="total_amount" class="form-control disabled text-right" placeholder="Tanggal invoice" value="<?=number_format($total_amount);?>" readonly>
											</div>
										</div>
										<div class="col-lg-4 col-md-4">	
											<div class="form-group">
												<label class="control-label">Tgl. Transfer*</label>
												<input type="text" name="tgl_transfer" id="tgl_transfer" class="form-control datepicker" placeholder="Tanggal pembayaran" value="<?=date("d-m-Y")?>" required <?=$disabled?>>
											</div>
										</div>
									</div>
									<div class="form-group">
													...
													<div class="col-lg-12 col-md-12">
														<div class="col-lg-5 col-md-5">
															<label class="control-label" onclick="show('ket<?=$id?>')">
																...
																<img src="<?=$abs?>/reg/img/<?=$row1['img']?>" width="40px" style="padding-bottom: 6px">
															</label>
														</div>
									...
									<div class="form-group">
										...
										<input type="file" name="bukti" id="bukti" class="form-control" required <?=$disabled?>>
									</div>
